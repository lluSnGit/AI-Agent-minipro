import { generateImageToVideo, getAccessToken, setAccessToken, clearTokens } from '../../utils/api'

Page({
  data: {
    image: '', // 图片预览路径
    imagePath: '', // 图片原始路径（用于上传）
    prompt: '', // 5秒视频提示词
  ...
  onShareAppMessage(options: WechatMiniprogram.Page.IShareAppMessageOption) {
    ...
    return {
      title: '图生视频 - AI智能体，基于图片生成动态视频',
      path: '/pages/image-to-video/image-to-video',
      imageUrl: firstImage || this.data.image || undefined
    }
  },
  // 用户点击右上角分享到朋友圈按钮时触发
  onShareTimeline() {
